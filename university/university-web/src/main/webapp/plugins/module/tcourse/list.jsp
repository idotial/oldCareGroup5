 ...
 <!-- content start -->
  <div class="admin-content">
 	<div class="up-cf up-padding">
      <div class="up-fl up-cf"><strong class="up-text-primary up-text-lg">TCourse</strong> / <small>TCourse</small></div>
    </div>
      ...
      <div class="up-u-sm-12">
        <form class="up-form" method="post" name="list_form">
          ...
          <c:forEach items="${list}" var="rowdata" varStatus="status">
            <tr >
                            <td><input type="checkbox" name="ids" value="${rowdata.courseId}" /></td>
              													<td title="${rowdata.courseName}" > <a href="${basePath}${path}view/${rowdata.courseId}" class="name">${rowdata.courseName}</a></td>
                									
																			<td >${rowdata.courseCost}</td>
													
																			<td >${rowdata.introduction}</td>
