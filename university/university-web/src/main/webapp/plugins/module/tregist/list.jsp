 ...
 <!-- content start -->
  <div class="admin-content">
 	<div class="up-cf up-padding">
      <div class="up-fl up-cf"><strong class="up-text-primary up-text-lg">TRegist</strong> / <small>TRegist</small></div>
    </div>
      ...
      <div class="up-u-sm-12">
        <form class="up-form" method="post" name="list_form">
          ...
          <c:forEach items="${list}" var="rowdata" varStatus="status">
            <tr >
                            <td><input type="checkbox" name="ids" value="${rowdata.registId}" /></td>
              													<td title="${rowdata.classId}" > <a href="${basePath}${path}view/${rowdata.registId}" class="name">${rowdata.classId}</a></td>
                									
																			<td >${rowdata.oldId}</td>
													
																		<td ><fmt:formatDate value="${rowdata.registTime}" type="both" pattern="yyyy-MM-dd"/></td>
