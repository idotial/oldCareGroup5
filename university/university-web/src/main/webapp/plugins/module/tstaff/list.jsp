 ...
 <!-- content start -->
  <div class="admin-content">
 	<div class="up-cf up-padding">
      <div class="up-fl up-cf"><strong class="up-text-primary up-text-lg">TStaff</strong> / <small>TStaff</small></div>
    </div>
      ...
      <div class="up-u-sm-12">
        <form class="up-form" method="post" name="list_form">
          ...
          <c:forEach items="${list}" var="rowdata" varStatus="status">
            <tr >
                            <td><input type="checkbox" name="ids" value="${rowdata.staffId}" /></td>
              													<td title="${rowdata.staffName}" > <a href="${basePath}${path}view/${rowdata.staffId}" class="name">${rowdata.staffName}</a></td>
                									
																			<td >${rowdata.phone}</td>
													
																			<td >${rowdata.idCard}</td>
													
																			<td >${rowdata.job}</td>
													
																		<td ><fmt:formatDate value="${rowdata.upData}" type="both" pattern="yyyy-MM-dd"/></td>
													
																		<td ><fmt:formatDate value="${rowdata.offData}" type="both" pattern="yyyy-MM-dd"/></td>
